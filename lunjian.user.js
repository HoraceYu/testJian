...
// 要使用的技能 //
var useSkills = '神龙东来；月夜鬼萧；燎原百击；冰月破魔枪；如来神掌；九溪断月枪；燎原百破；排云掌法；九天龙吟剑法；覆雨剑法；雪饮狂刀；翻云刀法；独孤九剑；玄铁剑法；辟邪剑法；天师剑法';
...
window.go = function (str) {
    var arr = str.split(";");
    // if (followTeam) {
    //     g_gmain.clickButton('team chat ' + go);
    // }
    ...
    if (cmd_stop == 0) {
        ...
        if (cmd.indexOf('trigger') != -1) {
            ...
            if (cmd.indexOf('yell') != -1) {
                ...
                clickButton(cmd);
                // if (followTeam) {
                //     g_gmain.clickButton('team chat ' + cmd);
                // }
            }
        } else {
            if (cmd.indexOf('yell') != -1) {
            ...
            case 'kill': {
                if (cmd_target_id) {
                    ...
                    if (cmd_target_id) {
                        ...
                        // if (followTeam) {
                        //     g_gmain.clickButton('team chat ' + cmd + ' ' + cmd_target_id);
                        // }
                    }
                }
            }; break;
...
(function () {
    ...
    var hairsfalling =
    {
        ...
        "峨眉山": {
            ...
            "青书少侠": "jh 8;w;nw;n;n;n;n;e;e;n;n;e;kill-看山弟子;n;n;n;n;n;e;e",
    ...
    var Jianshi = { zha: 0, xs: 0, dr: 0, chonglian: 0, qianlong: 0, tianjian: 0, jstimer: 0, qingzhengxie: 0, bangzhan: 0, bx: 0, bs: 0, sp: 0, sys: 1, renwu: 0, zhengxie: 0, teshu: 0, tf: 0, yx: 0, gw: 0, hd: 0, qx: 0, qxmj: 0, qxhg: 0, gensha: 0 };
    ...
    var Base = {
        ...
        skills: function () {
            ...
            if (this.getCorrectText('4253282')) {
                this.mySkillLists = '神龙东来；月夜鬼萧';
            }
            // 王有财
            if (this.getCorrectText('4219507')) {
                this.mySkillLists = '冰月破魔枪；九溪断月枪';
            }
            // 火狼
    ...
    function teamSay(e) {
        ...
        if (DomTxt == '队长说话') {
            ...
            $(document).on('click', '.cmd_click2', function () {
                ...
                if (text == '对话' || text == '比试' || text == '杀死' || text.indexOf('挑战') != -1) {
                    g_gmain.clickButton('team chat ' + clickText);
                }
                if (clickText.indexOf('event') != -1) {
                    g_gmain.clickButton('team chat ' + clickText);
                }
            });
    ...
    */
        ...
        waitIntervaltimer = setTimeout(() => {
            if (g_gmain.is_fighting) {
                ...
                if (xueshanTime) {
                    ...
                    if (hours > 9 && hours < 23) {
                        // 30秒后开杀
                        if (new Date().getTime() - xueshanTime > 1 * 3 * 1000) {
                            Base.mySkillLists = '万流归一；云梦归月；天魔妙舞';
                            clickButton(killtext);
                            console.log('已过3秒-开杀' + killtext);
                        }
                    } else {
                        // 29分后开杀
                        if (new Date().getTime() - xueshanTime > 29.2 * 60 * 1000) {
                            Base.mySkillLists = '万流归一；云梦归月；天魔妙舞';
                            clickButton(killtext);
                            console.log('已过29分-开杀' + killtext);
                        }
                    }
                } else {
                    xueshanTime = new Date().getTime();
                }
            } else {
                xueshanTime = 0;
            }
    ...
    function setKillXueTime() {
        ...
        var timer = prompt("请输入要使用的技能", timerText);
        if (timer) {
    ...
    let daimaText = 'ask tianlongsi_chaishao';
    function doDaiMa(e) {
        var Dom = $(e.target);
        var DomTxt = Dom.html();
        if (DomTxt == '代码定时') {
    ...
    function beforeFightTongren(e) {
        ...
        if (DomTxt == '脱衣') {
            ...
            Dom.html('脱衣');
            clickButton('auto_equip on');
            clickButton('enable map_all');
    ...
    function setNewSkillsPosition(type){
        ...
        if (skills.indexOf('月夜鬼萧') > -1) {
            ...
            skillsArr = ssetNewSkillsArr(skillsArr, '破军棍诀', type);
        } else if (skills.indexOf('千影百伤棍') > -1) {
            skillsArr = ssetNewSkillsArr(skillsArr, '千影百伤棍', type);
        }
    ...
    function answerQuestionsFunc(e) {
        ...
        if (Dom.html() == "答题") {
            console.log("准备自动答题！");
            answerQuestions();
            answerQuestionsInterval = setInterval(answerQuestions, 3000);
            Dom.html("停答题");
        } else {
            console.log("停止自动答题！");
            Dom.html("答题");
    ...
    function answerQuestions() {
        ...
        setTimeout(getAndAnsQuestion, 1500);
    }
    ...
    var QLHLocList = ['主页', '背包', '技能', '打榜', '监控Q群', '使用令牌', '打铁铺子', '桑邻药铺', '书房', '南市', '北大街', '钱庄', '绣楼', '祠堂大门', '厅堂', '杂货铺', '洛阳寺庙'];
    ...
    function makeOtherBtns() {
        ...
        for (var i = 0; i < QLHLocList.length; i++) {
            ...
            QLHchapMapButton[i].className = 'btn-add btn-others';
    ...
    function makeMoreBtns() {
        ...
        useSkillsButton.style.top = '576px';
        currentPos = Base.delta;
        ...
        findSpecTargetButton.className = 'btn-add btn-others';
        ...
        gSocketMsg.dispatchMessage = function (b) {
            ...
            if (type == "disconnect" && subType == "change") {
                if (!hasSendReload) {
                    ...
                    var sendtext = '';
                    if (nameObj[sendid]) {
                        sendtext = 'Duang,' + sendqu + '区，' + nameObj[sendid] + '被顶下线了!';
                        goInLine(sendtext);
                    }
    ...
    function doBang1(msg) {
        ...
        if (txt.indexOf('close') > 0) {
            var timeNum = 5* 60 * 1000;
            if (g_obj_map.get("msg_room").get("short") == "守将府" || g_obj_map.get("msg_room").get("short") == "南车道"){
                timeNum = 25 * 60 * 1000;
            }
            setTimeout(() => {
                openOnTime();
        ...
        if (obj.fromGroup != '291849393') {
            ...
            if (isKuaFu()) {
                ...
                if (obj.fromQQ == '35994480') {
                    ...
                    if (txt.indexOf('队长队伍') != '-1') {
                        goInLine("申请加入" + txt);
                        clickButton('team join u7594769')
                    }
                    ...
                    if (txt.indexOf('帮派副本2-') != '-1') {
                        goInLine("去" + txt);
                        let bangPaiFubenNum = txt.split('-')[1];
    ...
    function doOnTime() {
        ...
        if (hours == 7 || hours == 8 || hours == 9 || hours == 12 || hours == 20 || hours == 23) {
            // 钓鱼
            startDiaoYu();
        }
    }
            ...
            for (var i = 0; i < elements.length; i++) {
                if (elements[i].key.indexOf('skill') > -1) {
                    ...
                    if (val) {
                        ...
                        if (practice) {
                            if (valArr[1] && valArr[2] > 250 && valArr[2] < 500 && valArr[4] != 'known') {
    ...
    function tupoSpeed() {
        ...
        var selfTopoText = '神龙东来,冰月破魔枪,燎原百击,月夜鬼萧,紫虚辟邪剑,神剑慧芒,不凡三剑,天外飞仙,无剑之剑,释迦拈花指,降龙廿八掌,弹指神通,折花百式,无相六阳掌,天魔策,左手刀法,移花接玉刀,天刀八诀,火贪一刀,冰玄鞭法,飞鸿鞭法,降魔杖法,子母龙凤环,九星定形针,九字真言印,紫血大法,白首太玄经,龙象般若功,九阴逆,长春不老功,云梦归月,天魔妙舞,踏月留香,打狗棒法';
    ...
    function tupoSpeed1() {
        ...
        // go('items put_store tianlongsi_jingangsheli');
        // go('items get_store /obj/shop/tongtianwan');
        var selfTopoText = '移花接玉刀,左手刀法,天刀八诀,火贪一刀,天魔妙舞,神剑慧芒,不凡三剑,天外飞仙,无剑之剑,天魔策,弹指神通,折花百式,子母龙凤环,紫虚辟邪剑,长春不老功,九阴逆,紫血大法,白首太玄经,龙象般若功,降龙廿八掌,云梦归月,冰玄鞭法,九星定形针,九字真言印,飞鸿鞭法,降魔杖法,神龙东来,冰月破魔枪,燎原百击,月夜鬼萧';
        var topoArr = selfTopoText.split(',');
        // var idArr = getSkillIdArr(topoArr);
        setTimeout(() => {
    ...
    function tupoSkills2() {
        clickButton('skills');
        if (Base.getCorrectText('4253282')) {
            topoText = '移花接玉刀,左手刀法,天刀八诀,火贪一刀,天魔妙舞,神剑慧芒,不凡三剑,天外飞仙,无剑之剑,天魔策,紫虚辟邪剑,折花百式,神龙东来,冰月破魔枪,燎原百击,月夜鬼萧,打狗棒法,冰玄鞭法,子母龙凤环,九星定形针,释迦拈花指,降龙廿八掌,弹指神通,无相六阳掌,飞鸿鞭法,降魔杖法,九字真言印,紫血大法,白首太玄经,龙象般若功,九阴逆,长春不老功,云梦归月,踏月留香';
        }
    ...
    function skillPritice() {
        ...
        clickButton('skills');
        var selfTopoText = '秋风步法,魔刀决,回风拂柳剑,六阴追魂剑,小步玄剑,天罡指,金顶绵掌,逍遥掌,同归剑法,天山杖法,无常杖法,魔戟诀,一指禅,火蝠身法,轻烟飘渺掌,鹰爪擒拿手,魔钩诀,伏魔剑,养心拳,混元掌';
        var topoArr = selfTopoText.split(',');
    ...
    function isOneItem(name) {
        var itemArr = ['『秘籍木盒』', '玫瑰花', '高级乾坤袋', '乾坤袋', '突破丹礼包', '舞鸢尾', '狗粮', '保险卡'];
    ...
    function baoguoZhengli1Func() {
        ...
        zhengli("舞鸢尾", "obj_wuyiwei", "put_store");
            ...
            if (m.length > 1) {
                m.each(function () {
                    ...
                    if ($(that).text() == itemName) {
                        let SPANbao = $(that).parent().parent().find('span');
    ...
    function ZhanDouView() {
        this.dispatchMessage = function (b) {
            ...
            if (type == 'notice' || type == 'main_msg') {
                ...
                if (notice_msg.indexOf('你觉得体力耗尽') >= 0 && notice_msg.indexOf('便往洞外走去') >= 0) {
                    waKuang();
                }
                ...
                if (notice_msg.indexOf('获得') >= 0 && notice_msg.indexOf('锦鲤') > 0) {
                    // sendToQQ(notice_msg);
                ...
                if (notice_msg.indexOf('帮派副本') >= 0 && notice_msg.indexOf('完成') >= 0) {
                    sendToQQ(notice_msg);
                    setTimeout(() => {
                        sendToQQ('帮派副本完成，开始挂机');
                        openOnTime();
                        closeTianJian();
                    }, 2 * 60 * 1000);
                }
                //【帮派】地府-转轮王开启了帮派副本**十月围城**【二】
                if (notice_msg.indexOf('开启了帮派副本') != '-1' && notice_msg.indexOf('十月围城') != '-1') {
                    removeOnTime(1);
                    sendToQQ(notice_msg);
    ...
    function waKuang() {
        var roomName = $('#out .outtitle').text();
        var action = 'event_1_83697921';  // 地矿
        var action1 = 'event_1_64388826';  // 挖地矿
        if (killBadSwitch) {
            action = 'event_1_21731755';
        ...
        if (roomName.indexOf('矿洞入口') >= 0) {
            ...
            go('jh 2;n;n;n;n;n;n;n;n;n;n;w;w;event_1_85329567;w;w;event_1_21731755');
            go(action);
        }
        for (var i = 0; i < 5; i++) {
            go(action1);
    ...
    // 看舞
    function guanWu() {
        // clickButton('event_1_29896809', 1)  内力
    ...
    function goFastWay(e) {
        let wayArr = [
            ...
            // }, 
            {
                ...
                name: '花街-观舞',
                way: 'rank go 169;w;w;w;w;w;n;n;n;e;e;s;event_1_29896809 go'
            }, {
                name: '红螺寺',
                way: 'rank go 193'
            }, {
                ...
                name: '白猿',
                way: 'rank go 209;sw;s;s;s;s;s;s;s;s;s;s;s;w;w;n;n;n;n;nw;nw;nw;n;n;n'
            }, {
                name: '闯入冥庄',
    ...
    function addScreenBtn(arr) {
        ...
        for (var i = 0; i < arr.length; i++) {
            ...
            //a += "<br/><button onClick='go(\"" + goPath + "\")' style='background: #FF6B00; color: #fff; margin: 5px;'>【交密码】</button>";
            if (arr[i].type == 1) {
    ...
    window.goFindNpcInPlace = function (w, name, type) {
        ...
        let qlcookies = getCookie(qianlongCookieName);
        if (!qlcookies) {
            qlcookies = '{}';
        }
    ...
    function showQianlongWay() {
        var wayNameArr = ['雪亭镇', '洛阳', '华山村', '华山', '扬州', '丐帮', '乔阴县', '峨眉山', '恒山', '武当山',
            '晚月庄', '水烟阁', '少林寺', '唐门', '青城山', '逍遥林', '开封', '光明顶', '全真教', '古墓',
            '白驼山', '嵩山', '梅庄', '泰山', '铁血大旗门', '大昭寺', '黑木崖'];
        var arr = [];
        wayNameArr.forEach((item) => {
            arr.push({ name: item, click: 'goFindNpcInPlace("' + item + '", "测试呵呵", 3)', type: 1 });
        });
        addScreenBtn(arr);
    }
    ...
    function GetNPCPath(dir, name) {
        ...
        if (hasFindName) {
            ...
            sendToQQ('找到' + name + ',way:' + '"' + way.join(';') + '"');
            if (Jianshi.qianlong || Jianshi.zha) {
                console.log("发现NPC：" + name);
                isOnstep1 = false;
                steps = 0;
                killGoodSetInterval = setInterval(function () {
                    // killSet([name]);
    ...
    function autoBang1Way() {
        // let dir = 's;w;w;w;e;e;e;e;e;e;e;e;e;e;e;e;w;w;w;s;w;w;w;e;e;e;e;e;e;w;w;w;s;w;w;w;e;e;e;e;e;e;w;w;w;s;w;w;w;e;e;e;e;e;e;w;w;w;s;w;w;w;e;e;e;e;e;e;w;w;w;s';
        let dir = 's;e;e;e;e;e;e;e;e;e;w;w;w;w;w;w;e;e;e;s;w;w;w;e;e;e;e;e;e;w;w;w;s;w;w;w;e;e;e;e;e;e;w;w;w;s;w;w;w;e;e;e;e;e;e;w;w;w;s;w;w;w;e;e;e;e;e;e;w;w;w;s';
        let name = TianJianNPCList1;
            ...
            switch (g_obj_map.get("msg_room").get("map_id")) {
                ...
                case "luoyang":
                    if (g_obj_map.get("msg_room").get("obj_p") == "112") {
                        ...
                        miGongNavi.Append("草屋", "n;n;n;n;n;e;e;n;n;e;n");
                        miGongNavi.Append("游记货栈", "n;n;n;n;n;n;n;n;n;n;n;n;n;n;n;n;w;w;w;w;n;w");
                    } else {
                        if (g_obj_map.get("msg_room").get("short") == "矿洞入口") {
                            $("button[onclick*='event_1_83697921']").after('<button type="button" cellpadding="0" cellspacing="0" onclick="go(\'event_1_83697921;event_1_64388826;event_1_64388826;event_1_64388826;event_1_64388826;event_1_64388826;event_1_83697921;event_1_64388826;event_1_64388826;event_1_64388826;event_1_64388826;event_1_64388826;event_1_83697921;event_1_64388826;event_1_64388826;event_1_64388826;event_1_64388826;event_1_64388826;event_1_83697921;event_1_64388826;event_1_64388826;event_1_64388826;event_1_64388826;event_1_64388826;event_1_83697921;event_1_64388826;event_1_64388826;event_1_64388826;event_1_64388826;event_1_64388826;event_1_83697921;event_1_64388826;event_1_64388826;event_1_64388826;event_1_64388826;event_1_64388826;event_1_83697921;event_1_64388826;event_1_64388826;event_1_64388826;event_1_64388826;event_1_64388826;event_1_83697921;event_1_64388826;event_1_64388826;event_1_64388826;event_1_64388826;event_1_64388826;event_1_83697921;event_1_64388826;event_1_64388826;event_1_64388826;event_1_64388826;event_1_64388826;event_1_83697921;event_1_64388826;event_1_64388826;event_1_64388826;event_1_64388826;event_1_64388826;\')" class="cmd_click3"><span style="color:rgb(118, 235, 32)">地品*10</span></button>').after('<button type="button" cellpadding="0" cellspacing="0" onclick="go(\'event_1_83697921;event_1_64388826;event_1_64388826;event_1_64388826;event_1_64388826;event_1_64388826\')" class="cmd_click3"><span style="color:rgb(118, 235, 32)">地品矿洞</span></button>').remove();
                            $("button[onclick*='event_1_22034949']").after('<button type="button" cellpadding="0" cellspacing="0" onclick="go(\'event_1_22034949;event_1_40548659;event_1_40548659;event_1_40548659;event_1_40548659;event_1_40548659;event_1_22034949;event_1_40548659;event_1_40548659;event_1_40548659;event_1_40548659;event_1_40548659;event_1_22034949;event_1_40548659;event_1_40548659;event_1_40548659;event_1_40548659;event_1_40548659;event_1_22034949;event_1_40548659;event_1_40548659;event_1_40548659;event_1_40548659;event_1_40548659;event_1_22034949;event_1_40548659;event_1_40548659;event_1_40548659;event_1_40548659;event_1_40548659;event_1_22034949;event_1_40548659;event_1_40548659;event_1_40548659;event_1_40548659;event_1_40548659;event_1_22034949;event_1_40548659;event_1_40548659;event_1_40548659;event_1_40548659;event_1_40548659;event_1_22034949;event_1_40548659;event_1_40548659;event_1_40548659;event_1_40548659;event_1_40548659;event_1_22034949;event_1_40548659;event_1_40548659;event_1_40548659;event_1_40548659;event_1_40548659;event_1_22034949;event_1_40548659;event_1_40548659;event_1_40548659;event_1_40548659;event_1_40548659;\')" class="cmd_click3">普通*10</button>').after('<button type="button" cellpadding="0" cellspacing="0" onclick="go(\'event_1_22034949;event_1_40548659;event_1_40548659;event_1_40548659;event_1_40548659;event_1_40548659\')" class="cmd_click3">普通矿洞</button>').remove();
                            $("button[onclick*='event_1_21731755']").after('<button type="button" cellpadding="0" cellspacing="0" onclick="go(\'event_1_21731755;event_1_22920188;event_1_22920188;event_1_22920188;event_1_22920188;event_1_22920188;event_1_21731755;event_1_22920188;event_1_22920188;event_1_22920188;event_1_22920188;event_1_22920188;event_1_21731755;event_1_22920188;event_1_22920188;event_1_22920188;event_1_22920188;event_1_22920188;event_1_21731755;event_1_22920188;event_1_22920188;event_1_22920188;event_1_22920188;event_1_22920188;event_1_21731755;event_1_22920188;event_1_22920188;event_1_22920188;event_1_22920188;event_1_22920188;event_1_21731755;event_1_22920188;event_1_22920188;event_1_22920188;event_1_22920188;event_1_22920188;event_1_21731755;event_1_22920188;event_1_22920188;event_1_22920188;event_1_22920188;event_1_22920188;event_1_21731755;event_1_22920188;event_1_22920188;event_1_22920188;event_1_22920188;event_1_22920188;event_1_21731755;event_1_22920188;event_1_22920188;event_1_22920188;event_1_22920188;event_1_22920188;event_1_21731755;event_1_22920188;event_1_22920188;event_1_22920188;event_1_22920188;event_1_22920188;\')" class="cmd_click3"><span style="color:rgb(235, 218, 32)">天品*10</span></button></span></div>').after('<button type="button" cellpadding="0" cellspacing="0" onclick="go(\'event_1_21731755;event_1_22920188;event_1_22920188;event_1_22920188;event_1_22920188;event_1_22920188\')" class="cmd_click3"><span style="color:rgb(235, 218, 32)">天品矿洞</span></button></span></div>').remove();
                        }
                    }
                    break;
                case "changan":
    ...
    function addTuBtn(txt) {
        ...
        if (Jianshi.qianlong) {
            ...
            // 不到前几个
            if (qianlongIndex < 13 && CookiesNpc != '西门吹雪') {
                return false;
            }
            if (btnArr[0] != '铁血大旗门' && !btnArr[0].stype && btnArr[0] != '黑木崖') {
                goFindNpcInPlace(btnArr[0], NPCNAME);
            } else if (btnArr[1] != '铁血大旗门' && !btnArr[1].stype && btnArr[1] != '黑木崖') {
                goFindNpcInPlace(btnArr[1], NPCNAME);
            } else if (btnArr[2] != '铁血大旗门' && !btnArr[2].stype && btnArr[2] != '黑木崖') {
                            ...
                            if (e == 2 && autoPuzzle.puzzleWating && autoPuzzle.puzzleWating.puzzleid && /完成谜题\((\d+)\/\d+\)：(.*)的谜题\S*\s*\S*x(\d+)\s*\S*x\d+\s*\S*银两x(\d{1,})/.test(a)) {
                                ...
                                if (puzzleFinish[2] == autoPuzzle.puzzleList[autoPuzzle.puzzleWating.puzzleid].firstPublisherName) {
                                    ...
                                    if (a.indexOf('当前谜题密码') >= 0) {
                                        console.log(a);
                                        // a = '<span style="color:rgb(37, 167, 182)">完成谜题(2/15)：老狼的谜题，获得：<br/>经验x285048<br/>潜能x5700960<br/>银两x3067</span><span style="color:rgb(235, 218, 32)"><br/>当前谜题密码：603534</span><br/><a href='javascript:go1("cus|puzzlesubmit|huashancun_huashancun_fb7")'>【提交】</a>'
                                        //当前谜题密码：327945
